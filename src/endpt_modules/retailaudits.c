...
int process_retailaudits_batch(MYSQL *conn, const struct Endpoint *endpoint, 
                             struct json_object *batch, struct RetailAuditBatchResult *result) {
    // Initialize result
    memset(result, 0, sizeof(struct RetailAuditBatchResult));
    ...
    for (size_t i = 0; i < n_records; i++) {
        ...
        bool success = true;
        int retailauditid = process_retailaudit_record(conn, record);
        ...
        if (retailauditid <= 0) {
            ...
            if (json_object_object_get_ex(record, "Item", &items)) {
                if (process_retailaudit_items(conn, retailauditid, items) != 0) {
                    success = false;
                }
            }

            // Process custom fields if present
            struct json_object *customfields;
            if (json_object_object_get_ex(record, "CustomFields", &customfields)) {
                if (process_retailaudit_customfields(conn, retailauditid, customfields) != 0) {
                    success = false;
                }
            }
        }
        ...
        if (result->records_processed % 10 == 0) {
            printf("\rProcessing retail audits: %d/%zu records", 
